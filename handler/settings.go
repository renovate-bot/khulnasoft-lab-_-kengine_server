...
import (
	...
	api_messages "github.com/khulnasoft-lab/kengine_server/constants/api-messages"
	"github.com/khulnasoft-lab/kengine_server/model"
	"github.com/khulnasoft-lab/kengine_server/pkg/constants"
	"github.com/khulnasoft-lab/kengine_server/pkg/sendemail"
...
func (h *Handler) TestConfiguredEmail(w http.ResponseWriter, r *http.Request) {
	...
	err = emailSender.Send([]string{user.Email}, "Kengine Testmail", "This is a test email", "", nil)
	if err != nil {
		h.respondError(&InternalServerError{err}, w)
		return
	}
...
func (h *Handler) TestUnconfiguredEmail(w http.ResponseWriter, r *http.Request) {
	...
	// send email to user
	email := user.Email
	err = emailSender.Send([]string{email}, "Kengine Testmail", "This is a test email", "", nil)
	if err != nil {
...
const (
	startAgentScript     = "start_kengine_agent.sh"
	uninstallAgentScript = "uninstall_kengine_agent.sh"
	agentBinaryFileAmd64 = "kengine-agent-amd64-%s.tar.gz"
	agentBinaryFileArm64 = "kengine-agent-arm64-%s.tar.gz"
)
