...
import (
	...
	reportersScan "github.com/khulnasoft-lab/kengine_server/reporters/scan"
	reporters_search "github.com/khulnasoft-lab/kengine_server/reporters/search"
...
func StartMultiScan(ctx context.Context,
	...
	for _, req := range reqs {
		...
		err = ingesters.AddNewScan(ctx, tx,
			...
			isPriority,
			action,
			scanTriggerCommon.KengineSystemScan)
		...
		if err != nil {
			if e, is := err.(*ingesters.AlreadyRunningScanError); is {
				...
				continue
			} else if _, is = err.(*ingesters.KengineSystemScanError); is {
