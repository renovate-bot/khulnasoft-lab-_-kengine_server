...
type KengineSystemScanError struct {
	NodeID string
}

func (ve *KengineSystemScanError) Error() string {
	return fmt.Sprintf("kengine_system_scan=true required to scan Kengine image/pod/container: %s", ve.NodeID)
}
...
func AddNewScan(ctx context.Context,
	...
	res, err := tx.Run(ctx, fmt.Sprintf(`
		...
		RETURN n IS NOT NULL AS Exists, n.is_kengine_system`,
	...
	if !kengineSystemScan && rec.Values[1] != nil && rec.Values[1].(bool) {
		return &KengineSystemScanError{
			NodeID: nodeID,
		}
	}
	...
	case controls.Image:
		...
		ORDER BY rand()
		LIMIT 1
		MATCH (l:Node{node_id: "kengine-console-cron"})
